...
\begin{center}
	\begin{circuitikz}
		...
		{to[R,l_=$R\sub{A}$] ++(0,-2) node[ground](G){}}% from in+
		[\vout]%
...
\begin{center}
	\begin{circuitikz}
		...
		{to[short] ++(0,-0.5) node[ground](G){}}% from in+
		[\vout]
	\end{circuitikz}
\end{center}
...
\begin{center}
	\begin{circuitikz}
		\invertingopamp{0,0}
		{to[C,l_=$C$] ++(-2,0) to[R,l_=$R\sub{A}$] ++(-2,0) to[short,-o] ++(-0.5,0) node[above]{\vin}}% from in-
		[R=$R$]% feedback element
		{to[R,l_=$R\sub{B}$] ++(0,-2) node[ground](G){}}% from in+
		[\vout]%
		% Add additional feedback element
