...
\begin{center}
	\begin{circuitikz}
		...
		{to[R,l_=$R$] ++(-2,0) to[short,-o] ++(-0.5,0) node[above]{\vin}}% inverting input network starting at coordinate #1-in-
		{R=$R\sub{F}$}% feedback element
		{to[R,l_=$R\sub{A}$] ++(0,-2) node[ground](G){}}% non-inverting input network starting at coordinate #1-in+
		[\vout]%
