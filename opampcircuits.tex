...
% TODO: Fix the code for CF
\begin{center}
	\begin{circuitikz}
		\draw (0, 0) node[op amp] (opamp) {}
		(opamp.-) to[short,-*] ++(-1,0) coordinate(FB)
		(opamp.+) to[short] ++(-1,0) coordinate(B)
		(B) to[R=$R\sub{B}$] ++(0,-2) node[ground](G){}
		(opamp.out) to[short,*-o] ++(1,0) coordinate(C)
		(FB) to[C,l_=$C$] ++(-2,0) to[R,l_=$R\sub{A}$] ++(-2,0) to[short,-o] ++(-1,0) node[above]{\vin}
		(FB) to[short,-*] ++(0,1) coordinate(FL) |- ++(1,0) coordinate[yshift=1ex](L1) to[R=$R$] ++(2,0) coordinate(FR) -| (opamp.out) to[short] coordinate(OUT) ++(1,0) node[above]{\vout}
		(FL) to[short] ++(0,1.5) coordinate(L2) to[C=$C\sub{F}$] ++(2,0) to[short] ++(1.375,0) to[short,-*] ++(0,-1.475) |- (FR);
	\end{circuitikz}
\end{center}
...
Both of these problems are due to the fact that the gain increases as frequency increases -- the circuit's gain is theoretically infinite at very high frequencies, though a real op amp's bandwidth will limit the high frequency gain.
To address this a problem a practical differentiator introduces \R[A] and \C[F].
...
\textcolor{red}{
\begin{equation}
	\frac{\vout}{\vin}(s) = \frac{-sRC}{(1 + s R \C[F])(1 + s\R[A] C)}
	\label{eq:practical_differentiator_TF}
\end{equation}
}

% TODO: finish the discussion of the practical differentiator.
